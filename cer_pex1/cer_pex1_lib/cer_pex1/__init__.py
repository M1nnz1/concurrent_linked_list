...
def test_np_true(test_name, value: np.array):
    if np.all(value):
        print("\033[92mTest {}: passed.\033[0m".format(test_name))
    else:
        print("\033[91mTest {}: failed.\033[0m".format(test_name))

def test_np_equal(test_name, value: np.array, target: np.array):
    if np.all(value==target):
        print("\033[92mTest {}: passed.\033[0m".format(test_name))
    else:
        print("\033[91mTest {}: failed.\033[0m".format(test_name))

def test_np_almost_equal(test_name, value: np.array, target: np.array, precision: float = 1e-4):
    if np.max(np.abs(value-target)) < precision:
        print("\033[92mTest {}: passed.\033[0m".format(test_name))
    else:
        print("\033[91mTest {}: failed.\033[0m".format(test_name))

def test_np_almost_zero(test_name, value: float, precision: float = 1e-4):
    test_np_almost_equal(test_name, value, np.zeros(value.shape), precision=precision)

def test_np_gt(test_name, upper: np.array, lower: np.array, precision: float = 0.0):
    test_np_true(test_name, upper > lower - precision)

def test_np_geq(test_name, upper: np.array, lower: np.array, precision: float = 0.0):
    test_np_true(test_name, upper >= lower - precision)
